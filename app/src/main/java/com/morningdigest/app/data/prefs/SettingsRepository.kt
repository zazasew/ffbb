...
data class AppSettings(
    ...
    // Municipality used by the official Norwegian Police operational log.
    ...
    val policeCategories: Set<String> = com.morningdigest.app.data.remote.PoliceIncidentFetcher.CATEGORY_TRANSLATIONS.keys,
...
class SettingsRepository(private val context: Context) {
    ...
    val settingsFlow: Flow<AppSettings> = context.dataStore.data.map { prefs ->
        AppSettings(
            ...
            policeMunicipality = prefs[Keys.POLICE_MUNICIPALITY] ?: "Ringerike",
            policeAlertsEnabled = prefs[Keys.POLICE_ALERTS_ENABLED] ?: true,
            policeCategories = prefs[Keys.POLICE_CATEGORIES] ?: com.morningdigest.app.data.remote.PoliceIncidentFetcher.CATEGORY_TRANSLATIONS.keys,
    ...
    suspend fun updatePoliceSettings(municipality: String, enabled: Boolean, categories: Set<String>) {
        context.dataStore.edit {
            it[Keys.POLICE_MUNICIPALITY] = municipality
    ...
    suspend fun setPoliceSeenIds(ids: Set<String>) {
        context.dataStore.edit { it[Keys.POLICE_SEEN_IDS] = ids.takeLast(200).toSet() }
    }
